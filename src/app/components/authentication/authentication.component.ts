...
export class AuthenticationComponent implements OnInit {
    ...
    private _emailValidationRegex =
        /(?:[a-z0-9!#$%&'*+/=?^_`{|}~-]+(?:\.[a-z0-9!#$%&'*+/=?^_`{|}~-]+)*|"(?:[\x01-\x08\x0b\x0c\x0e-\x1f\x21\x23-\x5b\x5d-\x7f]|\\[\x01-\x09\x0b\x0c\x0e-\x7f])*")@(?:(?:[a-z0-9](?:[a-z0-9-]*[a-z0-9])?\.)+[a-z0-9](?:[a-z0-9-]*[a-z0-9])?|\[(?:(?:25[0-5]|2[0-4][0-9]|[01]?[0-9][0-9]?)\.){3}(?:25[0-5]|2[0-4][0-9]|[01]?[0-9][0-9]?|[a-z0-9-]*[a-z0-9]:(?:[\x01-\x08\x0b\x0c\x0e-\x1f\x21-\x5a\x53-\x7f]|\\[\x01-\x09\x0b\x0c\x0e-\x7f])+)\])/;
    private _passwordValidationRegex = /^(?=.*[a-z])(?=.*[A-Z])(?=.*\d)(?=.*[@$!%*?&])[A-Za-z\d@$!%*?&]{8,}$/;

    constructor(private _httpService: HttpService, private _userService: UserService, private _router: Router, private _messageService: MessageService) {}
    ...
    public register(registrationModel: IRegistrationModel) {
        console.log(this._passwordValidationRegex.test("Mark1997!"));

        const isValid = this._validateFields(registrationModel.email, registrationModel.password, true);
        console.log(isValid);
        ...
        this._httpService.registerUser(registrationModel).subscribe((res) => {
            if (res.state === HTTP_STATE.SUCCESS) {
                ...
                this._messageService.ErrorMessage("There was a problem with the registration process.");
            }
        });
    }

    //full validation determines weather only the length validation or the regex validation should also run
    private _validateFields(email: string, password: string, fullValidation: boolean = false): boolean {
        var isValid = true;

        if (this._validateEmail(email, fullValidation) && this._validatePassword(password, fullValidation)) {
            return true;
        }
        return false;
    }
    ...
    //full validation determines weather only the length validation or the regex validation should also run
    private _validateEmail(email: string, fullValidation: boolean): boolean {
        let isValid = true;
        if (!email) {
    ...
    //full validation determines weather only the length validation or the regex validation should also run
    private _validatePassword(password: string, fullValidation: boolean): boolean {
        let isValid = true;
        if (!password) {
            this._messageService.WarningMessage("Password can't be empty!");
            isValid = false;
        } else if (fullValidation && !this._passwordValidationRegex.test(password)) {
            this._messageService.WarningMessage("Password must be at least 8 characters long, and it must contain at least an upper case letter, a lower case letter a number and a special character!!");
            isValid = false;
        }
        return isValid;
    }
